    ...
    wiced_bt_a2dp_source_data_t *p_data)
{
    ...
    printf("wiced_bt_a2dp_source_get_ccb %d %s \r\n",event,wiced_bt_a2dp_source_evt_code(event));
#endif
    switch (event)
    {
    ...
    case WICED_BT_A2DP_SOURCE_API_START_EVT:
        WICED_BTA2DP_SRC_TRACE("Start req event %d handle \r\n",p_data->start_req.handle);
...
void wiced_bt_a2dp_source_hdl_event(uint8_t event, wiced_bt_avdt_evt_hdr_t *p_msg)
{
    ...
    wiced_bt_a2dp_source_ccb_t *p_ccb = NULL;
#if WICED_BT_A2DP_SOURCE_DEBUG
    printf("[wiced_bt_a2dp_source_hdl_event] with event %d  %s \r\n",event,wiced_bt_a2dp_source_evt_code(event));
#endif
    if( (event == WICED_BT_A2DP_SOURCE_INVALID_EVT) || (event >= WICED_BT_A2DP_SOURCE_LAST_EVENT) )
    {
#if WICED_BT_A2DP_SOURCE_DEBUG
        printf("wiced_bt_a2dp_source_hdl_event return empty as event is %d \r\n",event);
#endif
        return;
    }
    ...
    if(event < WICED_BT_A2DP_SOURCE_GLOBAL_EVT_END)
    {
        ...
        WICED_BTA2DP_SRC_TRACE("wiced_bt_a2dp_source_hdl_event-wiced_bt_a2dp_source_nsm_act \r\n");
    }
    else
    {
